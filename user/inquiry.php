...
$departments = $conn->query("
    SELECT d.id, d.name, 
           CASE 
               WHEN d.id IN (2,4,5) THEN 'Mayor RJ Peralta' 
               WHEN d.id IN (8) THEN 'VM Atty. Imee Cruz' 
               ELSE 'Other' 
           END as office
    FROM departments d
    WHERE d.id IN (2,4,5,8)
    ORDER BY 
        CASE 
            WHEN d.id IN (2,4,5) THEN 1 
            WHEN d.id IN (8) THEN 2 
            ELSE 3 
        END,
        d.id
                        ...
                        <div class="col-md-12">
                            ...
                            <select class="form-select" name="department_id" required>
                                ...
                                foreach ($departments as $dept): 
                                    if ($dept['office'] != $current_office):
                                        if (!empty($current_office)) echo '</optgroup>';
                                        $current_office = $dept['office'];
                                        echo '<optgroup label="'.$current_office.'">';
                                    endif;
                                ?>
