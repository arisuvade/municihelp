...
<div class="dashboard-header">
    <div class="dashboard-container">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h1 class="mb-1">MSWD Approved Requests</h1>
...
<div class="dashboard-container">
    ...
    <?php else: ?>
        ...
        <?php foreach ($regularRequests as $queueDate => $requests): ?>
            ...
            $isPastDue = (new DateTime($queueDate)) < (new DateTime('today'));
            ...
            ");
            $countStmt->bind_param("s", $queueDate);
            $countStmt->execute();
            ...
            <div class="day-header">
                <div>
                    <h4 class="mb-0">Request List - <?= $formattedDate ?>
                        <?php if ($isPastDue): ?>
                            <span class="badge bg-danger past-due-badge">Past Due</span>
                        <?php endif; ?>
                    </h4>
                    <div class="limit-info <?= $remaining <= 0 ? 'limit-reached' : '' ?>">
                ...
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-striped mb-0">
                            <thead>
                                <tr>
                                    <th class="table-col-queue">Queue #</th>
                                    <th class="table-col-name">Name</th>
                                    <th class="table-col-program">Program</th>
                            ...
                            <tbody>
                                ...
                                ?>
                                    <tr class="request-row" 
                                        ...
                                        data-queue-date="<?= $row['queue_date'] ?>"
                                        data-is-walkin="0">
                                        <td class="table-col-queue"><?= htmlspecialchars($row['queue_no'] ?? 'N/A') ?></td>
                                        <td class="table-col-name"><?= $applicant_name ?></td>
                                        <td class="table-col-program"><?= htmlspecialchars($row['program']) ?></td>
                                        ...
                                        <td class="table-col-actions">
                                            ...
                                            <button class="btn btn-reschedule reschedule-btn" data-id="<?= $row['id'] ?>">
                ...
                ?>
                    <div class="mobile-row request-row" 
                        ...
                        <div>
                            ...
                            <?= htmlspecialchars($row['queue_no'] ?? 'N/A') ?>
                        </div>
                        <div>
                            <span class="mobile-label">Name:</span>
                            <?= $applicant_name ?>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="recipientRelation" required>
                    </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    Are you sure you want to reschedule this request?
                    <div class="mt-3">
                        <label for="rescheduleDate" class="form-label">New Date:</label>
                        <input type="date" class="form-control" id="rescheduleDate">
                        <div id="rescheduleLimitInfo" class="limit-info mt-2">
                            Remaining slots: <strong>300</strong>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
    ...
    <div class="modal fade" id="successRescheduleModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-warning text-dark">
                    <h5 class="modal-title">Request Rescheduled</h5>
                </div>
                <div class="modal-body text-center">
                    <i class="fas fa-calendar-check fa-5x text-warning mb-4"></i>
                    <h4>Request Rescheduled Successfully!</h4>
    ...
    $(document).ready(function() {
        ...
        function checkDailyLimit(date, elementId, buttonId) {
            if (!date) {
                $(elementId).html('Please select a date');
                if (buttonId) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        const limitInfo = $(elementId);
                        
                        if (remaining <= 0) {
                            limitInfo.html(`<strong>Daily limit reached (${count}/${dailyLimit})</strong><br>
                                          Cannot reschedule more requests for this date.`);
                        ...
                        $(elementId).html('Error checking daily limit');
                    }
                },
        ...
        // Check limit when reschedule date changes
        $('#rescheduleDate').on('change', function() {
            const selectedDate = $(this).val();
            checkDailyLimit(selectedDate, '#rescheduleLimitInfo', '#confirmReschedule');
        });
        
        // Check limit when reschedule modal opens
        $('#rescheduleModal').on('show.bs.modal', function() {
            // Set default date to tomorrow
            const tomorrow = new Date();
            tomorrow.setDate(tomorrow.getDate() + 1);
            const tomorrowFormatted = tomorrow.toISOString().split('T')[0];
            $('#rescheduleDate').val(tomorrowFormatted);
            
            // Check limit for tomorrow
            checkDailyLimit(tomorrowFormatted, '#rescheduleLimitInfo', '#confirmReschedule');
        });
        ...
        $(document).on('click', '.view-btn', function() {
            ...
            $.ajax({
                url: '../../../mayor/mswd/view_request.php?id=' + requestId,
                method: 'GET',
        ...
        $(document).on('click', '.reschedule-btn', function() {
            ...
            if (currentIsWalkin) {
                $('#rescheduleMessage').text('Walk-in request rescheduled.');
            } else {
                $('#rescheduleMessage').text('The applicant has been notified via SMS.');
            }
        ...
        $('#confirmReschedule').click(function() {
            ...
            const limitInfoText = $('#rescheduleLimitInfo').text();
            if (limitInfoText.includes('Daily limit reached')) {
                $validation.removeClass('d-none').text('Daily limit reached for this date. Cannot reschedule.');
                return;
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        $validation.removeClass('d-none').text(response.error || 'Failed to reschedule request');
                    }
                },
                error: function(xhr) {
                    $validation.removeClass('d-none').text('Error: ' + (xhr.responseJSON?.error || xhr.statusText));
        ...
        $('#continueAfterComplete, #continueAfterDecline, #continueAfterReschedule').click(function() {
            location.reload();
        });
...
function applyFilters() {
    ...
    $('.request-row').each(function() {
        ...
        // Fix program filter - check both program and parent program
        ...
        if (matchesSearch && matchesProgram && matchesBarangay && matchesDate) {
            ...
            if (isDesktop) {
                $row.closest('.desktop-table').show();
                $row.closest('.desktop-table').prev('.day-header').show();
            } else {
                $row.closest('.mobile-table').show();
                $row.closest('.mobile-table').prev('.day-header').show();
            }
            $row.show();
            anyVisible = true;
        } else {
