...
// Get superadmin info
$superadminId = $_SESSION['mayor_superadmin_id'];
$superadminQuery = $GLOBALS['conn']->query("SELECT name FROM admins WHERE id = $superadminId");
$superadminData = $superadminQuery->fetch_assoc();
$superadminName = $superadminData['name'] ?? 'Superadmin';
...
// Determine if this is a scheduled report (monthly) or adhoc
$isScheduled = isset($_POST['is_scheduled']) && $_POST['is_scheduled'] == '1';
$reportsDir = $isScheduled 
    ? '../../reports/mayor/animal/scheduled/' 
    : '../../reports/mayor/animal/adhoc/';
...
// Generate filename
$fileName = isset($_POST['filename']) ? $_POST['filename'] : 'animal_report_' . date('Y-m');
$response = ['success' => true];

try {
    // Get ALL report data with filters (no admin filtering)
    $data = getReportData($_POST);

    // Process PDF if requested
    if (isset($_POST['generate_pdf']) && $_POST['generate_pdf'] == '1') {
        $pdfFilename = $fileName . '.pdf';
        generatePDF($data, $superadminName, $_POST['start_date'], $_POST['end_date'], $pdfFilename);
    ...
    if (isset($_POST['generate_excel']) && $_POST['generate_excel'] == '1') {
        ...
        generateExcel($data, $superadminName, $_POST['start_date'], $_POST['end_date'], $excelFilename);
        $response['file'] = $reportsDir . $excelFilename;
    }
} catch (Exception $e) {
    error_log("Report Generation Error: " . $e->getMessage());
    $response = ['success' => false, 'error' => 'Report generation failed'];
}

echo json_encode($response);

function getReportData($postData) {
    global $conn;
    ...
    foreach ($results as $row) {
        ...
        if (!empty($postData['program']) && $postData['program'] !== '') {
        ...
        // Apply status filter
        if (!empty($postData['status']) && $postData['status'] !== '') {
            $statusMatch = false;
            
            // Special handling for false_report
            if ($postData['status'] === 'false_report') {
                if ($row['status'] === 'false_report' || $row['status'] === 'False Report') {
                    $statusMatch = true;
                }
            } 
            // Case-insensitive comparison for other statuses
            elseif (strtolower($row['status']) === strtolower($postData['status'])) {
                $statusMatch = true;
            }
