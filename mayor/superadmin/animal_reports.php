...
// Function to get scheduled reports with the pattern animal_report_YYYY-MM
function getScheduledReports($dir) {
    $reports = [];
    if (file_exists($dir)) {
        $files = scandir($dir, SCANDIR_SORT_DESCENDING);
        foreach ($files as $file) {
            if (preg_match('/^animal_report_(\d{4})-(\d{2})\.(pdf|xlsx)$/', $file, $matches)) {
                $year = $matches[1];
                $month = $matches[2];
                $baseName = "animal_report_{$year}-{$month}";
...
$scheduledReports = getScheduledReports('../../reports/mayor/animal/scheduled/');
?>
...
<head>
    ...
    <style>
        :root {
            ...
            --light-color: #f8f9fa;
            --dark-color: #212529;
            --ambulance-red: #4CAF50;
            --ambulance-red-light: #ffccd5;
        }
    ...
    <div class="dashboard-header">
        <div class="dashboard-container">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h1 class="mb-1">Animal Reports</h1>
                    <p class="mb-0">Generate comprehensive animal service reports</p>
                </div>
...
$(document).ready(function() {
    ...
    // Report generation function for animal requests
    ...
    $('.report-card').click(function() {
        ...
        Swal.fire({
            ...
            html: `Downloading both report files for ${fileName.replace('animal_report_', '').replace('_', ' ')}`,
            confirmButtonColor: '#4CAF50',
            timer: 3000,
            timerProgressBar: true
        });
