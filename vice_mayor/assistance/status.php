...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cancel_request'])) {
    ...
    // Start transaction for assistance request cancellation
    $conn->begin_transaction();
    
    try {
        // Get the file paths first before cancellation
        $stmt = $conn->prepare("SELECT specific_request_path, indigency_cert_path, id_copy_path, id_copy_path_2, request_letter_path FROM assistance_requests WHERE id = ? AND user_id = ? AND status = 'pending'");
        ...
        if ($request) {
            ...
            $filesDeleted = deleteRequestFiles('assistance_requests', $requestId, $conn);
            
            // Clear file references from database
            $referencesCleared = clearFileReferences('assistance_requests', $requestId, $conn);
            
            // THEN update request status
            $stmt = $conn->prepare("UPDATE assistance_requests SET status = 'cancelled', reason = 'Cancelled by user' WHERE id = ?");
            ...
            if (!$filesDeleted || !$referencesCleared) {
                $_SESSION['warning_message'] = 'Request cancelled but some files may not have been deleted properly';
            }
        } else {
            $_SESSION['error_message'] = 'Unable to cancel request - request not found or already processed';
            $conn->rollback();
        }
        ...
        error_log("Error cancelling assistance request: " . $e->getMessage());
    }
