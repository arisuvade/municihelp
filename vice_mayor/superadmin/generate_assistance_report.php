...
// Get superadmin info
$superadminId = $_SESSION['vice_mayor_superadmin_id'];
...
$fileName = isset($_POST['filename']) ? $_POST['filename'] : 'assistance_report_' . $timestamp;
...
if (isset($_POST['generate_pdf']) && $_POST['generate_pdf'] == '1') {
    try {
        ...
        generatePDF($data, $superadminName, $_POST['start_date'], $_POST['end_date'], $pdfFilename, isset($_POST['with_amount']) && $_POST['with_amount'] == '1');
...
if (isset($_POST['generate_excel']) && $_POST['generate_excel'] == '1') {
    try {
        ...
        generateExcel($data, $superadminName, $_POST['start_date'], $_POST['end_date'], $excelFilename, isset($_POST['with_amount']) && $_POST['with_amount'] == '1');
...
function getReportData($postData) {
    ...
    $query = "SELECT 
              ...
              AND (ar.status != 'cancelled' OR ar.cancelledby_admin_id IS NOT NULL)
...
function generatePDF($data, $adminName, $startDate, $endDate, $filename, $withAmount) {
    ...
    // Calculate summary counts - REMOVE PENDING COUNT
    ...
    // REMOVED: $pendingOnline = count(array_filter($onlineRequests, function($item) { return $item['status'] === 'pending'; }));
    ...
    $html = '<div style="font-family: Arial; line-height: 1.5;">
        <h2 style="text-align:center;">Assistance Comprehensive Report</h2>
