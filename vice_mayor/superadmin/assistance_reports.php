...
$superadmin_query = $conn->query("SELECT name FROM admins WHERE id = $vice_mayor_superadmin_id");
$superadmin_data = $superadmin_query->fetch_assoc();
$superadmin_name = $superadmin_data['name'] ?? 'Superadmin';

$pageTitle = 'Assistance Reports';
...
$statuses = ['approved', 'declined', 'completed', 'cancelled'];

// Function to get scheduled reports with the pattern assistance_report_YYYY-MM
function getScheduledReports($dir) {
    $reports = [];
    if (file_exists($dir)) {
        $files = scandir($dir, SCANDIR_SORT_DESCENDING);
        foreach ($files as $file) {
            if (preg_match('/^assistance_report_(\d{4})-(\d{2})\.(pdf|xlsx)$/', $file, $matches)) {
                $year = $matches[1];
                $month = $matches[2];
                $baseName = "assistance_report_{$year}-{$month}";
...
$scheduledReports = getScheduledReports('../../reports/vice_mayor/assistance/scheduled/');
?>
...
<body>
    <div class="dashboard-header">
        <div class="dashboard-container">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h1 class="mb-1">Assistance Reports</h1>
                    <p class="mb-0">Generate comprehensive assistance reports</p>
                </div>
    ...
    <div class="dashboard-container">
        <!-- Assistance Request Report Section -->
        <div class="recent-activity-card card mb-4">
            <div class="card-header">
                <h4 class="mb-0">Generate Assistance Request Report</h4>
            </div>
            <div class="card-body">
                <form id="reportForm">
                    <div class="row mb-3">
...
$(document).ready(function() {
    ...
    // Report generation function for assistance requests
    async function generateReport(withAmount) {
        ...
        try {
            ...
            const pdfResponse = await $.ajax({
                url: 'generate_assistance_report.php',
                method: 'POST',
    ...
    $('.report-card').click(function() {
        ...
        Swal.fire({
            ...
            html: `Downloading both report files for ${fileName.replace('assistance_report_', '').replace('_', ' ')}`,
            confirmButtonColor: '#4CAF50',
            timer: 3000,
            timerProgressBar: true
        });
